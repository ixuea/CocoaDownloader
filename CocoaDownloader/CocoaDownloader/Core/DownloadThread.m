...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler{
    ...
    downloadLogDebug(@"download thread didReceiveResponse size:%ld",self.downloadInfo.size);
    
    //cache path
//    self.downloadInfo.cachePath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:self.downloadInfo.id];
//
//    //获取文件管理器
//    NSFileManager *manager = [NSFileManager defaultManager];
//
//    if (![manager fileExistsAtPath:self.downloadInfo.cachePath]) {
//        //如果文件不存在，就创建一个文件
//        [manager createFileAtPath:self.downloadInfo.cachePath contents:nil attributes:nil];
//    }
//
//    //创建文件句柄
//    self.fileHandle = [NSFileHandle fileHandleForWritingAtPath:self.downloadInfo.cachePath];
    
    //获取文件管理器
    NSFileManager *manager = [NSFileManager defaultManager];
    
    if (![manager fileExistsAtPath:self.downloadInfo.path]) {
        //截取文件夹路径
        NSString *downloadFolder=[self.downloadInfo.path stringByDeletingLastPathComponent];
        
        //创建文件夹
        [manager createDirectoryAtPath:downloadFolder withIntermediateDirectories:YES attributes:nil error:nil];
        
        //创建文件
        [manager createFileAtPath:self.downloadInfo.path contents:nil attributes:nil];
    }
    
    //创建文件句柄
    self.fileHandle = [NSFileHandle fileHandleForWritingAtPath:self.downloadInfo.path];
